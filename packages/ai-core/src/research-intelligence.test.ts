...
describe("research tool registry (deep)", () => {
  ...
  it("describes deep as deterministic orchestration without an agent loop", () => {
    expect(researchToolDescription("builtin:research.deep")).toMatch(/deterministic/i);
    const params = researchToolParameters("builtin:research.deep") as {
      required: string[];
  ...
  it("rates deep risk as medium", () => {
    expect(researchRiskFor("deep", false)).toBe("medium");
  });
});
